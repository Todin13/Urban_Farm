...
SensorData JsonSerializer::deserialize(const web::json::value& jsonValue) {
    SensorData data;
    data.sensorID = jsonValue[U("sensor_ID")].as_integer();
    data.sensorVersion = jsonValue[U("sensor_Version")].as_string();
    data.temperature = jsonValue[U("Temperature")].as_double();
    data.humidity = jsonValue[U("Humidity")].as_double();
    data.plantID = jsonValue[U("Plant_ID")].as_integer();
